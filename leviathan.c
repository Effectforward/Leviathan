...
/*It uses yt-dlp to parse the link and download the video/playlist*/
    ...
    */
    switch (choice) {
        case 1:
            sprintf(command, "yt-dlp -f \"bestvideo[height<=240]+bestaudio/best\" --ignore-errors --retries infinite --force-ipv4 --merge-output-format mp4 \"%s\"", url);
            break;
        case 2:
            sprintf(command, "yt-dlp -f \"bestvideo[height<=360]+bestaudio/best\" --ignore-errors --retries infinite --force-ipv4 --merge-output-format mp4 \"%s\"", url);
            break;
        case 3:
            sprintf(command, "yt-dlp -f \"bestvideo[height<=480]+bestaudio/best\" --ignore-errors --retries infinite --force-ipv4 --merge-output-format mp4 \"%s\"", url);
            break;
        case 4:
            sprintf(command, "yt-dlp -f \"bestvideo[height<=720]+bestaudio/best\" --ignore-errors --retries infinite --force-ipv4 --merge-output-format mp4 \"%s\"", url);
            break;
        case 5:
            sprintf(command, "yt-dlp -f \"bestvideo[height<=1080]+bestaudio/best\" --ignore-errors --retries infinite --force-ipv4 --merge-output-format mp4 \"%s\"", url);
            break;
        case 6:
            sprintf(command, "yt-dlp -f \"bestvideo+bestaudio/best\" --ignore-errors --retries infinite --force-ipv4 --merge-output-format mp4 \"%s\"", url);
            break;
            default:
            choice = 0;
            sprintf(command, "yt-dlp\"%s\"", url);
    }
...
scanf("%d", &choice);
  ...
  {
  case 1:
    sprintf(command, "spotdl download \"%s\" --log-level DEBUG --save-file \"spotdl_progress.spotdl\" --user-auth", url);
  break;
  
  
case 2:
    sprintf(command, "spotdl download \"%s\" --log-level DEBUG --save-file \"spotdl_progress.spotdl\"", url);
    break;
    
    default:
    //no flag just simple download 
    sprintf(command, "spotdl download \"%s\"", url);
    break;
